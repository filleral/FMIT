...
class  Media {
 ...
 public function process(){
    ...
    elseif(!is_writable($this->productPath)):
      $this->errors[] = $this->productPath."Oye, Debes tener permisos de escritura!!!.";
      return false;
    elseif(file_exists($this->productPath."/".$this->fileName)):
      $this->errors[] = "El archivo {$this->fileName} este Articulo ya esta en la nube :(";
      return false;
    else:
     return true;
    endif;
 ...
 public function process_user($id){
    ...
    if(!is_writable($this->userPath)){
        $this->errors[] = $this->userPath."Oye, Debes tener permisos de escritura";
        return false;
      }
    if(!$id){
...
/* Función para eliminar medios por id.*/
   public function media_destroy($id,$file_name){
     ...
     if(delete_by_id('media',$id)){
       ...
       $this->error[] = "Se ha producido un error en la eliminación de la imagen.";
       return false;
     }
